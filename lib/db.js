...
function completeBooking(id, paymentMethod, tipAmount, completedBy, money) {
  ...
  db.prepare(
    ...
    "amount_charged = ?, amount_paid = ?, discount_amount = ? WHERE id = ?"
  ).run(
...
function getPayoutSummary(therapistId, startDate, endDate) {
  ...
  return {
    ...
    totalOwed: round(serviceShare + cardTips),
    lines,
  };
}
...
function recordPayout(p) {
  return db.prepare(
    ...
    "service_share, card_tips, cash_tips, total_paid, notes, paid_by) " +
    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
  ).run(p.therapistId, p.startDate, p.endDate, p.sessions || 0, p.serviceTotal || 0,
        p.serviceShare || 0, p.cardTips || 0, p.cashTips || 0, p.totalOwed || 0,
        p.notes || "", p.paidBy || "");
}
...
module.exports = {
  ...
  priceForTherapist, getTherapistPrices, setTherapistPrice,
  getPayoutSummary, recordPayout, getPayoutHistory, noShowBooking, getBookingById, getAvailableSlots,
