...
function addExpense(opts) {
  return db.prepare(
    "INSERT INTO expenses (description, amount, category, date, frequency, is_startup, vendor, notes, payment_status, paid_by, due_to, due_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
  ).run(
...
function updateExpense(id, data) {
  db.prepare(`
    UPDATE expenses SET description=?, amount=?, category=?, date=?, frequency=?, is_startup=?, vendor=?, notes=?, payment_status=?, paid_by=?, due_to=?, due_date=?
    WHERE id=?
  `).run(
...
module.exports = {
  ...
  getExpenses, addExpense, updateExpense, deleteExpense, getExpenseById, markExpensePaid, getExpenseTotals, getRecurringExpenseSummary,
  resetTestData,
