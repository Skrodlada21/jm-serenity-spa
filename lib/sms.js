...
async function sendReminderSMS(booking) {
  ...
  // "Reminder" rather than "tomorrow": the lead time is an admin setting
  // (sms_reminder_hours), so a hardcoded "tomorrow" would be a lie the moment
  // anyone sets it to 48. The exact day is on the next line anyway.
  const msg =
    `${spaName} - reminder\n` +
...
async function sendDailyReminders() {
  ...
  // How far ahead to remind is an admin setting (Settings -> Text Reminders).
  // It used to be editable but ignored, so changing it silently did nothing.
  // Hours are converted to whole days because reminders are selected by DATE;
  // 24 -> tomorrow, 48 -> two days out, and anything under 24 still means
  // tomorrow (we never text about appointments already in progress today).
  const reminderHours = parseInt(settings.sms_reminder_hours, 10);
  const daysAhead = Math.max(1, Math.round((isNaN(reminderHours) ? 24 : reminderHours) / 24));

  const target = new Date();
  target.setDate(target.getDate() + daysAhead);
  const tomorrowStr = ymdLocal(target);
