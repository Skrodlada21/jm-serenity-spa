...
async function sendBookingConfirmation(booking, addons, baseUrl) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p style="color: #999; font-size: 12px;">${spaName} &bull; ${settings.address || "Littleton, CO"} &bull; ${settings.phone || ""}</p>
    </div>
  `;
...
async function sendCancellationEmail(booking) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>If you'd like to rebook, visit <a href="${siteUrl}/book">our booking page</a>.</p>
      <hr style="border: none; border-top: 1px solid #eee; margin: 1.5rem 0;" />
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="text-align: center; padding: 1rem; background: #f9f9f9; border-radius: 0 0 12px 12px; border: 1px solid #eee; border-top: none;">
        <p style="color: #999; font-size: 12px; margin: 0;">${spaName} &bull; ${settings.address || "Littleton, CO"}</p>
      </div>
    </div>
  `;
