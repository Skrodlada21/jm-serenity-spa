...
/**
 * Did Google itself verify this message? A From header alone proves nothing.
 * We look at the Authentication-Results Google stamped on delivery.
 */
function passedAuth(headerValue) {
  if (!headerValue) return false;
  const h = String(headerValue).toLowerCase();
  return h.includes("spf=pass") || h.includes("dkim=pass");
}
