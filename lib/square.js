...
async function createTerminalCheckout(amountCents, note, options = {}) {
  ...
  const checkoutRequest = {
    ...
    checkout: {
      ...
      referenceId: options.referenceId ? String(options.referenceId)
        : options.bookingId ? String(options.bookingId) : "",
    },
  };
...
/**
 * Fetch a completed payment (the tip lives on the Payment, not the checkout).
 *
 * @param {string} paymentId - Square payment ID from checkout.paymentIds
 * @returns {object|null} - The payment object
 */
async function getPayment(paymentId) {
  ...
  try {
    ...
    console.error("Square get payment error:", err.message || err);
    return null;
  }
}

/**
 * List the account's paired devices (used to find the Terminal's device_id).
 *
 * No location filter: the legacy SDK builds a broken query string when only
 * locationId is passed, and the account will only have our own devices anyway.
 *
